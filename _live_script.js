	...
	function loadShopConfig() {
	  fetch(API + 'shop-config').then(function(r){ return r.json(); }).then(function(d) {
	    if (d.bannerText) {
	...
	function loadProducts() {
	  api('shop/products', null, 'GET').then(function(d) {
	...
	function doSellerLogin() {
	  ...
	  api('shop/login', { email: email, password: pass }).then(function(d) {
	...
	function loadSellerSettings() {
	  api('shop/config', {}).then(function(d) {
	...
	function uploadSellerLogo() {
	  ...
	  fetch(API + 'shop/upload-logo', { method: 'POST', headers: { shopToken: shopToken }, body: fd })
	...
	function uploadSellerQr() {
	  ...
	  var fd = new FormData(); fd.append('file', file);
	  fetch(API + 'shop/upload-qr', { method: 'POST', headers: { shopToken: shopToken }, body: fd })
	  .then(function(r) { return r.json(); }).then(function(j) {
	    if (j.error) throw new Error(j.error);
	    toast('QR đã upload!');
	    loadSellerSettings();
	...
	function addSellerProduct() {
	  ...
	  var doAdd = function(imageUrl) {
	    api('shop/add-product', { name: name, description: desc, price: price, category: cat, imageUrl: imageUrl || '' }).then(function() {
	  ...
	  if (file) {
	    var fd = new FormData(); fd.append('file', file);
	    fetch(API + 'shop/upload-product-image', { method: 'POST', headers: { shopToken: shopToken }, body: fd })
    .then(function(r) { return r.json(); }).then(function(j) {
      if (j.error) throw new Error(j.error);
      doAdd(j.url);
    }).catch(function(e) { toast(e.message, true); });
	...
	function deleteSellerProduct(id) {
	  ...
	  api('shop/delete-product', { productId: id }).then(function() {
	...
	(function init() {
	  ...
	  loadProducts();
	  if (shopToken) {
	    api('shop/config', {}).then(function(){}).catch(function() {
      shopToken = '';
      localStorage.removeItem('shopToken');
    });
	  }
